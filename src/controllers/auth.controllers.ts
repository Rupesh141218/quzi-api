...
export const login = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    passport.authenticate("local", { session: false }, (err: any, user: IUser | IAdmin | IGoogleUser, info: { message: any }) => {
      if (err) {
        return next(err)
      }
 ...
 * Admin login
 */
export const adminLogin = (req: Request, res: Response, next: NextFunction): void => {
  try {
    // Validate input
    const validation = validateInput(loginSchema, req.body)
    ...
    passport.authenticate("admin", { session: false }, (err: any, admin:IAdmin, info: { message: any }) => {
      if (err) {
        return next(err)
      }
